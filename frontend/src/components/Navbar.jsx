...
const Navbar = () => {
  ...
  const { navigate, setShowSearch, getCartCount, token, setToken,  setCartItems } = useContext(ShopContext)
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium'>
      ...
      <div className='flex items-center gap-6'>
        <img onClick={() => setShowSearch(true)} src={assets.search_icon} alt="icon" className='w-5 cursor-pointer' />

        <div className='group relative'>
          <img onClick={() => token ? null : navigate('/login')} src={assets.profile_icon} alt="icon" className='w-5 cursor-pointer'/>
          {/* Dropdown Menu  */}
          {token && 
        ...
        <Link to='/cart' className="relative">
            <img src={assets.cart_icon} alt="icon" className='min-w-5 w-5'/>
            <p className='absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-black text-white aspect-square rounded-full text-[8px]'>{getCartCount()}</p>
        </Link>
        <img onClick={() => setShowMenu(true)} src={assets.menu_icon} alt="icon" className='w-5 cursor-pointer sm:hidden' />
      </div>
        ...
        <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${showMenu ? 'w-full' : 'w-0'}`}>
            <div className='flex flex-col text-gray-600 '>
                <div onClick={() => setShowMenu(false)} className='flex items-center gap-4 p-3 cursor-pointer'>
                  <img className='h-4 rotate-180' src={assets.dropdown_icon} alt="icon" />
                  <p>Back</p>
                </div>
